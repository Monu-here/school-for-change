...
class SmFeesController extends Controller
{
    ...
    public function searchFeesPayment(Request $request)
    {
        ...
        return view('backEnd.feesCollection.search_fees_payment', compact('students'));
    }

    public function paidFeeByStudet($id)
    {
        $unpaiditems = [];
        $paiditems = [];
        ...
        return response()->json(['paidfee' => $paiditems, 'unpaidfee' => $unpaiditems]);
    }
    ...
    {
        $students = SmStudent::all();
        return view('backEnd.feesCollection.search_fees_due', compact('students'));
    }
    ...
    public function balanceFeesSearch(Request $request)
    {
        ...
        foreach ($students as $student) {
            ...
            $total_discount = 0;
            $total_amount = 0;
            if (SmBillIssue::where('student_id', $student->id)->where('due', '>', '0')->count() > 0) {
                $total_amount = SmBillIssue::where('student_id', $student->id)->where('due', '>', '0')->sum('total');
                $total_balance = SmBillIssue::where('student_id', $student->id)->where('due', '>', '0')->sum('due');
                $balance_students[$student->id] = ['name' => $student->full_name, 'rollno' => $student->roll_no, 'total' => $total_amount, 'due' => $total_balance, 'fathername' => $student->parents->fathers_name];
            }
        }
