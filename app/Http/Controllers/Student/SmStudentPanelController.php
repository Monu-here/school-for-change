...
class SmStudentPanelController extends Controller
{
    ...
    public function studentDashboard(Request $request, $id = null)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
         ...
         $unpaiditems = [];
         $paiditems = [];
         $duebill = SmBillIssue::where('student_id', $student_detail->id)->get();
        ...
        //  dd($unpaiditems);
        $bills = Bill::where('student_id',$student_detail->id)->get();

        return view('backEnd.studentPanel.my_profile', compact('driver', 'student_detail', 'fees_assigneds', 'fees_discounts', 'exams', 'documents', 'timelines', 'siblings', 'grades','unpaiditems','paiditems','bills'));
    }
    ...
    public function studentResult()
    {
        ...
        $data = SmResultStore::where('student_id',$student_detail->id)->get();
        // dd($data);
        $datas=[];
        foreach($data as $r){
            if(!isset( $datas[$r->exam_Type_id]['name'])){
                $ex_type=\App\SmExamType::find($r->exam_type_id);
                $datas[$r->exam_Type_id]['name']=$ex_type->title;
                $datas[$r->exam_Type_id]['results']=[];

            }
            
            $line=[];
            $line[$r->id]=$r->toArray();
            $line[$r->id]['subject']=\App\SmSubject::find($r->subject_id)->subject_name;

            $line[$r->id]['partial']=[];
            foreach($r->partials() as $partial ){
                $p=[];
                $p['mark']=$partial->marks;
                $f=$partial->fullmark();
                $p['full']=  $f->exam_mark;
                $p['pass']=  $f->passmark;
                array_push( $line[$r->id]['partial'],$p);
            }
            array_push( $datas[$r->exam_Type_id]['results'],$line);
        }
        // dd($datas);


      
        return view('backEnd.studentPanel.student_result', compact('student_detail', 'datas'));
    }
