    ...
    <div class="row">
        <div class="col-md-4" >
            <div style="background:white; padding:10px; border-radius:5px;">
                <form action="{{ route('fiscal_update',$fiscal->id) }}" method="POST">
                    @csrf
                    <div class="form-group">
                        <label for="title">Title</label>
                        <input type="text" name="title" value="{{ $fiscal->name }}" placeholder="Enter title" class="form-control" required>
                    </div>

                    <div class="form-group">
                        <label for="title">Start Date</label>
                        <input type="date" name="start_date" class="form-control" value="{{ $fiscal->start_date }}" required>
                    </div>

                    <div class="form-group">
                        <label for="title">End Date</label>
                        <input type="date" name="end_date" class="form-control" value="{{ $fiscal->end_date }}" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        "> Is Active ?
                        <input type="hidden" name="is_active" id="is_active" value="1" {{ $fiscal->is_default == 1 ? 'checked' : ''}}>
                    </div>
                    <hr>
                    <div class="form-group">
